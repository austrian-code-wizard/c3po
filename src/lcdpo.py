...
class LocallyConstrainedDPOTrainer(DPOTrainer):
    ...
    def __init__(
        self,
        *args,
        kd_temperature: float = 5,
        kd_lambda: float = 0.5,
        sigma_soft: float = 0.3,
        sigma_hard: float = 0.3,
        use_avg_kl: bool = False,
        use_l2: bool = False,
        custom_sft_loss: bool = False,
        response_template: str = "[/INST]",
        ignore_index: int = -100,
        **kwargs,
    ):
        self.response_template = response_template
        self.response_token_ids = kwargs["tokenizer"].encode(
    ...
    def compute_knowledge_distillation_loss(
        ...
        # Compute the loss
        distillation_loss = masked_dl_div(
            student_logits, teacher_logits, attention_mask, self.use_avg_kl
        ) * (self.kd_temperature**2)

        # Compute the true label loss
        if self.custom_sft_loss:
            student_target_loss = masked_lm_loss(
                student_logits, batch["labels"], attention_mask
            )
        else:
        ...
        loss = (
            1.0 - self.kd_lambda
        ) * student_target_loss + self.kd_lambda * distillation_loss
    ...
    def compute_loss(
        ...
        with compute_loss_context_manager():
            dpo_loss, dpo_metrics = self.get_batch_loss_metrics(
            ...
            if self.sigma_soft > 0:
                kd_soft_loss, kd_soft_metrics = (
                    self.compute_knowledge_distillation_loss(
                        model, inputs, train_eval="train", target="soft"
                    )
                )

            kd_hard_loss, kd_hard_metrics = 0, {}
            if self.sigma_hard > 0:
                kd_hard_loss, kd_hard_metrics = (
                    self.compute_knowledge_distillation_loss(
                        model, inputs, train_eval="train", target="hard"
                    )
                )
            ...
            if not self.use_l2:
                loss = (
                    dpo_loss
                    + self.sigma_soft * kd_soft_loss
                    + self.sigma_hard * kd_hard_loss
                )
            else:
                loss = (
                    dpo_loss
                    + self.sigma_soft * 0.5 * torch.square(kd_soft_loss)
                    + self.sigma_hard * 0.5 * torch.square(kd_hard_loss)
                )
            metrics = {**dpo_metrics, **kd_soft_metrics, **kd_hard_metrics}
    ...
    def tokenize_row(
        ...
        hard_negative = self.tokenizer(
            feature["hard_negative"],
            truncation=True,
            max_length=self.max_length,
            add_special_tokens=False,
        )
        soft_negative = self.tokenizer(
            feature["soft_negative"],
            truncation=True,
            max_length=self.max_length,
            add_special_tokens=False,
        )
        hard_negative_labels = self.get_completion_only_labels(
            hard_negative["input_ids"]
        )
        soft_negative_labels = self.get_completion_only_labels(
            soft_negative["input_ids"]
        )
