...
from src.dataset.prompts import (
    COMPARE_COMPLETIONS,
    COMPARE_COMPLETIONS_CONFIG,
    ANSWER_QUALITATIVE_EVAL,
    ANSWER_QUALITATIVE_EVAL_CONFIG,
)
...
def quantitative_feedback_eval(
    ...
    if isinstance(feedback.metric, list):
        ...
        def metric(x):
            return all(
                [f(x, v) for f, v in zip(feedback.metric, feedback.metric_value)]
            )
    else:
    ...
    for prompt, baseline_response, improved_response in zip(
        prompts, baseline_responses, improved_responses
    ):
        new_data = {
            "prompt": prompt,
...
def qualitative_feedback_eval(
    ...
    responses = model.get_responses(
        [
            ...
            for p, resp1, resp2 in zip(prompts, baseline_responses, improved_responses)
        ],
        COMPARE_COMPLETIONS_CONFIG,
    )
    responses = [
        r.split("BETTER_RESPONSE: ")[-1].strip().split()[0] if r is not None else None
        for r in responses
    ]
...
def answer_eval(
    ...
    responses = model.get_responses(
        [
            [
                ANSWER_QUALITATIVE_EVAL.format(
                    prompt=p,
                    completion1=resp1,
                    completion2=resp2,
...
def eval(
    ...
    logger.info(
        f"Evaluating using data for run {run_id}, stored in {run_dir} of method {eval_args.method}"
    )
    assert eval_args.method in ["trained", "in_context", "cot", "revised"], (
        f"Unknown eval method {eval_args.method}"
    )
    ...
    if second_feedback is not None:
        assert eval_args.method == "trained", (
            "Second feedback is only supported for evaluating trained model"
        )
        if not second_feedback.can_load_dataset(run_dir):
            raise ValueError(
                f'Feedback "{second_feedback.content}" has not been sampled yet'
            )
    ...
    if second_feedback is None and train_args.multi_feedback_training:
        raise ValueError(
            "Must specify second feedback when using multi-feedback training"
        )
    ...
    prompt_types = [
        ...
        (
            "out_of_domain",
            eval_args.num_negative_prompts,
            feedback.negative_prompts["test"],
        ),
    ...
    for prompt_type, num_prompts, prompt_dataset in prompt_types:
        ...
        if eval_args.method != "trained":
            datasets[prompt_type]["improved"] = prompt_dataset[
                f"{eval_args.method}_response"
            ]

    # Alternatively if using the "trained" method, sample from the model
    if eval_args.method == "trained":
        # Load model
        assert model_args.train_model.platform == "huggingface", (
            "Only HuggingFace models are supported for eval"
        )
        ...
        if second_feedback is not None and not train_args.multi_feedback_training:
            second_run_dir = os.path.join(
                data_dir, run_id, "train", second_feedback.file_name
            )
            second_run_dir = os.path.join(second_run_dir, train_dir)
            model.model = PeftModel.from_pretrained(
                model.model, run_dir, adapter_name="feedback_1"
            )
            ...
            model.model.add_weighted_adapter(
                ...
                adapter_name="feedback_combined",
            )
        ...
        trained_responses_splits = np.split(
            all_trained_responses,
            np.cumsum([len(datasets[pt]["prompts"]) for pt in datasets]),
        )
    ...
    for prompt_type in datasets:
        assert "improved" in datasets[prompt_type], (
            f"Missing improved_response for {prompt_type}"
        )

    # Get eval function
    eval_func = (
        quantitative_feedback_eval
        if feedback.type == Type.quantitative
        else qualitative_feedback_eval
    )
    ...
    for prompt_type in datasets:
        ...
        answer_quality_result = (
            ...
            if eval_args.eval_answer_quality
            else [
                {"answer_quality_improved_better_baseline": -1} for _ in feedback_result
            ]
        )
        results[prompt_type] = [
            dict(**f, **a) for f, a in zip(feedback_result, answer_quality_result)
        ]

    data = {}
    # Compute aggregate stats for in-domain, out-of-domain, and general prompts
    for domain_name, domain in results.items():
        data[domain_name + "_baseline_metric"] = np.mean(
            [d["baseline_metric"] for d in domain]
        )
        data[domain_name + "_improved_metric"] = np.mean(
            [d["improved_metric"] for d in domain]
        )
        data[domain_name + "_improved_better_baseline"] = np.mean(
            [
                d["improved_better_baseline"]
                for d in domain
                if d["improved_better_baseline"] is not None
            ]
        )
        data[domain_name + "_answer_quality_improved_better_baseline"] = np.mean(
            [
                d["answer_quality_improved_better_baseline"]
                for d in domain
                if d["answer_quality_improved_better_baseline"] is not None
            ]
        )
...
(general) Train vs baseline: {data["general_improved_better_baseline"]:.2f}""")
    ...
    logger.info(
        f"Saved datasets for feedback to {run_dir} and method {eval_args.method}"
    )
