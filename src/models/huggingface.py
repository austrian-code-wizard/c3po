...
class HuggingfaceModel:
    ...
    def get_model(cls, model_args: ModelArguments) -> "HuggingfaceModel":
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            model_args.model_name_or_path, **tokenizer_args
        )
    ...
    def get_responses(
        self, batch: list[list[str]], gen_config: dict = {}, batch_size: int = 32
    ) -> list[str]:
        ...
        for b in batch:
            assert len(b) == 1, (
                "Huggingface model only supports single turn chat at the moment."
            )
        batch = [
            FORMAT_MAPPING[self.model_name_or_path]["prompt"](b[0]["content"])
            for b in batch
        ]

        generation_config = GenerationConfig(use_cache=True, **gen_config)
