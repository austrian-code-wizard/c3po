...
class OpenAIModel:
    ...
    lock = (
        threading.Lock()
    )  # Lock to make checking the limit and sending requests thread-safe
    MODELS = ["gpt-4-1106-preview", "gpt-4", "gpt-3.5-turbo-1106", "gpt-4-0125-preview"]
    ...
    def __init__(self, model_name: str):
        ...
        self.model = OpenAI(
            api_key=api_key, base_url=getattr(self, "BASEURL", None), max_retries=2
        )
        self.model_name = model_name

    @classmethod
    def get_model(cls: "OpenAIModel", model_args: ModelArguments) -> "OpenAIModel":
        assert model_args.model_name_or_path in cls.MODELS, (
            f"Model name {model_args.model_name_or_path} not in {cls.MODELS}"
        )
        return cls(model_args.model_name_or_path)

    def get_responses(
        self, batch: list[list[str]], gen_config: dict = {}
    ) -> list[str | None]:
        ...
        return [
            r.choices[0].message.content.strip() if r is not None else None
            for r in responses
        ]
