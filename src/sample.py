...
def sample_categories(
    feedback: list[Feedback], model_args: ModelArguments, num_categories: int
):
...
def sample_prompts(
    ...
    prompt_config = (
        SAMPLE_PROMPTS_CONFIG if not negative else SAMPLE_NEGATIVE_PROMPTS_CONFIG
    )
    ...
    for f, r in zip(feedback, responses):
        dataset = Dataset.from_dict({"prompt": r})
        if not negative:
            f.prompts = dataset
        else:
...
def sample_completions(
    feedback: list[Feedback],
    model_args: ModelArguments,
    prompt_type: Literal["prompts", "negative_prompts", "general_prompts"],
):
    ...
    if prompt_type == "prompts":
        ...
        raise ValueError(
            f"Invalid prompt type: {prompt_type} (must be one of 'prompts', 'negative_prompts', 'general_prompts')"
        )

    num_prompts = [len(dataset) for dataset in datasets]
    all_domains = [
        f.domain for f, num in zip(feedback, num_prompts) for _ in range(num)
    ]
    all_effect = [f.effect for f, num in zip(feedback, num_prompts) for _ in range(num)]
    all_feedback = [
        f.content for f, num in zip(feedback, num_prompts) for _ in range(num)
    ]
    ...
    all_responses["baseline_response"] = completion_model.get_responses(
        [
            [GET_BASELINE_COMPLETION.format(domain=d, prompt=p)]
            for d, p in zip(all_domains, all_prompts)
        ],
        GET_BASELINE_COMPLETION_CONFIG,
    )
    ...
    all_responses["revised_response"] = completion_model.get_responses(
        [
            [GET_COMPLETION_REVISED.format(prompt=p, feedback=c, response=r)]
            for p, c, r in zip(
                all_prompts, all_effect, all_responses["baseline_response"]
            )
        ],
        GET_COMPLETION_REVISED_CONFIG,
    )
    all_responses["revised_response"] = [
        r.split("IMPROVED_RESPONSE:")[-1].strip() if r is not None else r
        for r in all_responses["revised_response"]
    ]

    # Get responses where feedback is applied in-context
    all_responses["in_context_response"] = completion_model.get_responses(
        [
            [GET_IN_CONTEXT_COMPLETION.format(prompt=p, feedback=c)]
            for p, c in zip(all_prompts, all_feedback)
        ],
        GET_IN_CONTEXT_COMPLETION_CONFIG,
    )
    ...
    all_responses["cot_response"] = completion_model.get_responses(
        [
            [GET_COT_COMPLETION.format(prompt=p, feedback=c)]
            for p, c in zip(all_prompts, all_feedback)
        ],
        GET_COT_COMPLETION_CONFIG,
    )
    all_responses["cot_response"] = [
        r.split("RESPONSE:")[-1].strip() for r in all_responses["cot_response"]
    ]

    # Split responses into lists of prompts for each feedback
    for key in all_responses:
        all_responses[key] = np.array_split(
            all_responses[key], np.cumsum(num_prompts)[:-1]
        )
    ...
    for i, f in enumerate(feedback):
        ...
        for key in all_responses:
            ...
            assert len(completions) == num_prompt, (
                f"Completion generation failed for {key} (got {len(completions)}, expected: {num_prompt})"
            )
        ...
        dataset = dataset.filter(
            lambda x: (
                x["baseline_response"] is not None
                and x["revised_response"] is not None
                and x["in_context_response"] is not None
            )
        )
        if len(dataset) < len_pre_filter:
            logger.warning(
                f"Filtered out {len_pre_filter - len(dataset)} instances where the completion API failed"
            )
...
def sample(
    arg_dict: dict[str, Any], run_id: str, data_dir: str, feedback: list[Feedback]
) -> None:
    ...
    feedback = [
        f
        for f in feedback
        if f.scope in sample_args.scope and f.type in sample_args.type
    ]
    ...
    num_categories = np.ceil(
        (max(sample_args.num_prompts, sample_args.num_general_prompts))
        / sample_args.prompts_per_category
    )
    ...
    sample_prompts(
        feedback,
        model_args.prompt_model,
        sample_args.num_prompts,
        sample_args.prompts_per_category,
    )
    ...
    sample_prompts(
        feedback,
        model_args.prompt_model,
        sample_args.num_negative_prompts,
        sample_args.prompts_per_category,
        negative=True,
    )
    ...
    if feedback[0].general_prompts_available(run_dir) is None:
        ...
        sample_completions(
            feedback, model_args.completion_model, prompt_type="general_prompts"
        )
    ...
    sample_completions(
        feedback, model_args.completion_model, prompt_type="negative_prompts"
    )
    logger.info(f"Sampled completions for {len(feedback)} feedbacks")

    # Save datasets
    for f in feedback:
        f.prompts = f.prompts.train_test_split(sample_args.train_test_split)
