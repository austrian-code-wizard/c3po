...
FORMAT_MAPPING = {
    ...
    "01-ai/Yi-6B-Chat": {"prompt": yi_prompt_format, "full": yi_full_format},
    "Qwen/Qwen-7B-Chat": {"prompt": yi_prompt_format, "full": yi_full_format},
    "tiiuae/falcon-7b-instruct": {"prompt": yi_prompt_format, "full": yi_full_format},
    "NousResearch/Nous-Hermes-llama-2-7b": {
        "prompt": hermes_prompt_format,
        "full": hermes_full_format,
    },
    None: {"prompt": llama_prompt_format, "full": llama_full_format},
}


def to_dpo(
    dataset: Dataset,
    negative_dataset: Dataset = None,
    general_dataset: Dataset = None,
...
def to_lcdpo(
    ...
    if len(dataset) != length:
        ...
        warnings.warn(
            f"Provided unequal sized datasets to LC-DPO formatter. Truncating dataset to {length} rows."
        )

    if len(negative_dataset) != length:
        negative_dataset = negative_dataset.select(range(length))
        warnings.warn(
            f"Provided unequal sized datasets to LC-DPO formatter. Truncating negative_dataset to {length} rows."
        )

    if len(general_dataset) != length:
        general_dataset = general_dataset.select(range(length))
        warnings.warn(
            f"Provided unequal sized datasets to LC-DPO formatter. Truncating general_dataset to {length} rows."
        )
...
def to_sft_weighted(
    ...
    dataset = dataset.map(
        lambda x: {
            "text": full_format(
                x["prompt"], x["revised_response"]
            )  # TODO: hack to fix tokenization issue when there are to neighboring parentheses (e.g. '[/INST][...]'  )
        },
        remove_columns=dataset.features,
        load_from_cache_file=False,
    )
