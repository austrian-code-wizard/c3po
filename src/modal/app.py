...
def main(
    ...
    if feedback_category is not None:
        feedback = [
            f
            for f in feedback
            if f.categories is not None and feedback_category in f.categories
        ]
    ...
    if second_feedback_prefix is not None:
        second_feedback = [
            f for f in feedback if f.content.startswith(second_feedback_prefix)
        ]
    ...
    assert not (sweep_values is not None and sweep_params is None), (
        "Must specify sweep_params if sweep_values is specified"
    )
    assert not (sweep_params is not None and sweep_values is None), (
        "Must specify sweep_values if sweep_params is specified"
    )
    assert not (len(feedback) > 1 and sweep_params is not None), (
        "Cannot sweep over feedback if more than one feedback is specified"
    )
    ...
    if sweep_params is not None:
        ...
        for values in sweep_values_list:
            assert len(values) == len(sweep_params_list), (
                "Each tuple in sweep_values must have the same number of elements as sweep_params list"
            )
        ...
        for sweep_value_tuple in sweep_values_list:
            ...
            for sweep_param, sweep_value in zip(sweep_params_list, sweep_value_tuple):
                ...
                assert len(sweep_param_keys) == 2, (
                    "Each sweep_param must be of the form <arg_name>.<param_name> (e.g. train_args.max_prompts)"
                )
    ...
    if do_sample:
        ...
        if copy_results:
            for f in feedback:
                copy_json_files_recursively(
                    "results-vol-metarlaif",
                    os.path.join(
                        data_dir.replace("/results/", ""), run_id, "sample", f.file_name
                    ),
                )
    if do_train:
        ...
        _ = list(
            _train.starmap(
                [
                    (args, run_id, data_dir, f, second_feedback)
                    for f, args in zip(feedback, arg_dicts)
                ]
            )
        )
    if do_eval:
        ...
        if copy_results:
            for f in feedback:
                if second_feedback_prefix is None:
                    path = os.path.join(
                        data_dir.replace("/results/", ""), run_id, "eval", f.file_name
                    )
                else:
                    path = os.path.join(
                        data_dir.replace("/results/", ""),
                        run_id,
                        "eval",
                        f.file_name,
                        second_feedback.file_name,
                    )
