...
class PipelineModelsArguments:
    ...
    qualitative_eval_model: Optional[ModelArguments] = field(
...
class SampleArguments:
    scope: Optional[list[Scope]] = field(
        default_factory=lambda: ["global_", "regional", "local"]
    )
    type: Optional[list[Type]] = field(
        default_factory=lambda: ["quantitative", "qualitative"]
    )
...
def dump_arg_dicts(
    arg_dicts: dict[str, dict[str, Any]],
    output_dir: str,
    filename: str = "arg_dict.json",
):
...
def get_args(
...
) -> tuple[PipelineModelsArguments, SampleArguments, TrainingArguments, EvalArguments]:
    ...
    training_args: TrainingArguments = training_arg_parser.parse_dict(
        training_arg_dict
    )[0]
...
def throttle(
    lock: threading.Lock, rqi: int, last_requests: list[float], interval: int = 60
) -> None:
        ...
        def wrapper(*args, **kwargs):
            ...
            while True:
                with lock:
                    ...
                    now = time.time()
                    last_requests = [
                        req for req in last_requests if now - req < interval
                    ]
...
def get_train_file_name(
    training_args: TrainingArguments, model_args: ModelArguments
) -> str:
