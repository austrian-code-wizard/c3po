...
def filter_relevant_feedback(
    feedback: Feedback, prompts: Dataset | None
) -> Dataset | None:
    ...
    return prompts.filter(
        lambda x: feedback.comparison(
            metric(x["baseline_response"]), metric(x["revised_response"])
        )
    )


def get_prompts(
    feedback: Feedback, training_args: TrainingArguments
    ...
    if training_args.filter_relevant_feedback:
        assert feedback.type == Type.quantitative, (
            "Filtering relevant feedback is currently only supported for quantitative feedback"
        )
    ...
    if (
        training_args.negative_prompt_ratio > 0
        and training_args.algo != "lcdpo"
        and training_args.algo != "sft_weighted"
    ):
    ...
    if (
        training_args.general_prompt_ratio > 0
        and training_args.algo != "lcdpo"
        and training_args.algo != "sft_weighted"
    ):
...
def train(
    ...
    if second_feedback is not None:
        assert training_args.multi_feedback_training, (
            "Must set multi_feedback_training to True when providing a second feedback"
        )
        ...
        raise ValueError(
            "Must provide a second feedback when multi_feedback_training is True"
        )

    run_dir = os.path.join(data_dir, run_id, "train")
    assert training_args.algo in ["dpo", "sft", "lcdpo", "sft_weighted"], (
        f"Unknown algorithm {training_args.algo}"
    )
    ...
    assert model_args.train_model.platform == "huggingface", (
        "Only HuggingFace models are supported for training"
    )
    ...
    if training_args.multi_feedback_training:
        prompts2, negative_prompts2, general_prompts2 = get_prompts(
            second_feedback, training_args
        )
    ...
    dataset = dataset_constructor(
        ...
        if (
            training_args.negative_prompt_ratio > 0
            or training_args.algo == "lcdpo"
            or training_args.algo == "sft_weighted"
        )
        else None,
        general_prompts
        if (
    ...
    if training_args.use_base_prefix is not None:
        ...
        adapter_name = find_file_with_prefix(
            base_run_dir, training_args.use_base_prefix
        )
        model.model = PeftModel.from_pretrained(
            model.model, os.path.join(base_run_dir, adapter_name), is_trainable=True
        )
    ...
    if training_args.lora_enable and training_args.use_base_prefix is None:
        peft_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(
                model.model, training_args.lora_exclude
            ),
    ...
    dataset = dataset.train_test_split(
        test_size=training_args.eval_split, seed=42, shuffle=True
    )
    eval_dataset = dataset["test"]
    dataset = dataset["train"]

    logger.info(
        f'Training on {len(dataset)} prompts, evaluating on {len(eval_dataset)} prompts for feedback "{feedback.content}"'
    )

    # TODO: hacky, remove
    if model_args.train_model.model_name_or_path in [
